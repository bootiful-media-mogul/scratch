...
//    }
    ...
    public Collection<Podcast> getAllPodcastsByMogul(Long mogulId) {
        return this.db.sql("select * from podcast where mogul_id =?").param(mogulId).query(new PodcastRowMapper()).list();
    }

    @Override
    public Collection<Episode> getEpisodesByPodcast(Long podcastId) {
        var podcast = getPodcastById(podcastId);
        Assert.notNull(podcast, "the podcast with id [" + podcastId +
                "] is null");
        return db.sql("select * from podcast_episode where podcast_id =? ").param(podcastId).query(episodeRowMapper).list();
    }
    ...
    public Podcast createPodcast(Long mogulId, String title) {
        ...
        this.db.sql("insert into podcast (mogul_id, title) values (?,?) on conflict on constraint  podcast_mogul_id_title_key do update set title =excluded.title").params(mogulId, title).update(kh);
        var id = (Number) kh.getKeys().get("id");
        return getPodcastById(id.longValue());
    }

    @Override
    public Episode createPodcastEpisode(Long podcastId, String title, String description, ManagedFile graphic, ManagedFile introduction, ManagedFile interview) {
        var kh = new GeneratedKeyHolder();
        this.db.sql("insert into podcast_episode(podcast_id, title, description,  graphic ,  introduction ,interview ) VALUES (?,?,?,?,?,? )")
                .params(podcastId,
                        title,
                        description,
                        graphic.toString(),
...
class PodcastRowMapper implements RowMapper<Podcast> {
    ...
    public Podcast mapRow(ResultSet rs, int rowNum) throws SQLException {
        return new Podcast(
                rs.getLong("mogul_id"),
                rs.getLong("id"), rs.getString("title"), rs.getDate("created"));
    }
}
