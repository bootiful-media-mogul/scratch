...
class PodcastDeletionIntegrationFlowConfiguration {
	...
	@Bean
	IntegrationFlow deletePodcastIntegrationFlow(MogulSecurityContexts mogulSecurityContexts, MogulService mogulService,
			Storage storage, PodbeanClient podbeanClient) {

		var messageSource = (MessageSource<Collection<Podcast>>) () -> MessageBuilder
			.withPayload(mogulService.getDeletedPodcasts())
			.build();
		...
		return IntegrationFlow//
			...
			.handle((GenericHandler<Podcast>) (payload, headers) -> {
				...
				podbeanClient.updateEpisode(payload.podbean().id(), payload.title(), payload.description(),
				...
				for (var uri : s3ToDelete) {
					...
					log.info("going to s3 delete [" + bucketName + "] and [" + folderAndFile + "] for podcast ["
							+ payload.id() + "] ");
