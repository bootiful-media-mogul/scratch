...
/*
    @Bean
    IntegrationFlow integrationFlow( ManagedFileService mfs,
            @Qualifier(PRODUCTION_FLOW_REQUESTS) MessageChannel channel) {
        return IntegrationFlow
                .from(channel)
                .handle((payload, headers) -> {
                    if (payload instanceof Episode ep)
                   return doWrite( log, )
                    return null;
                })
                .get();
...
*/
    ...
    IntegrationFlow episodeProductionIntegrationFlow(
        ...
        return IntegrationFlow
                ...
                .handle((GenericHandler<Map<String, String>>) (payload, headers) -> {
                    var managedFile = doWrite(log, episodeIdHeaderName, podcastService, managedFileService,
                            headers, payload);
                    return MessageBuilder
                            .withPayload(managedFile)
    ...
    private static ManagedFile doWrite(
            Logger log,
            String episodeIdHeaderName,
            PodcastService podcastService, ManagedFileService managedFileService,
        ...
        var episodeId = episodeIdValue instanceof String episodeIdString ?
                Long.parseLong(episodeIdString) : ((Number) episodeIdValue).longValue();
        ...
        try {
            ...
            throw new RuntimeException("could not download and then re-upload the file [" +
                    tmp.getAbsolutePath() + "] for episode [" + episode + "]",
                    e);
        } //
        finally {
