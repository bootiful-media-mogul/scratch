	...
	ApplicationRunner sampleArchiveDebugRunner(ArchiveConverter archiveConverter, ApiProperties properties) {
		return args -> {
			if (properties.debug()) {
				...
				var pa = new PodcastArchive(uuid, "the title for UUID (" + uuid + ")",
						"this is a sample podcast that i created for UUID (" + uuid + ")",
						new FileSystemResource(new File(sampleArchive, "intro.mp3")),
						new FileSystemResource(new File(sampleArchive, "interview.mp3")),
						new FileSystemResource(new File(sampleArchive, "image.jpg")));
				var file = new File(archives, uuid + ".tmp");
				try (var out = new FileOutputStream(file)) {
					archiveConverter.serialize(pa, out);
					Assert.state(file.exists() && file.length() > 0, "the archive should exist");
					log.debug("wrote the archive to " + file.getAbsolutePath() + '.');
				}
				Files.move(file.toPath(), new File(file.getParentFile(), file.getName() + ".zip").toPath());
			}
		};
	}
